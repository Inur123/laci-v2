 <header class="sticky top-0 z-50 border-b border-slate-200/60 bg-white/75 backdrop-blur" x-data="{
     ...
     linkBase() { return 'rounded-xl px-3 py-2 text-sm font-semibold transition'; },
     linkActive() { return 'bg-[#f0fdf4] text-[#166534] ring-1 ring-[#bbf7d0]'; },
     linkIdleDesktop() { return 'text-slate-600 hover:bg-slate-100'; },
     linkIdleMobile() { return 'text-slate-700 hover:bg-slate-100'; },
     ...
     <nav class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
         ...
         <div class="hidden items-center gap-2 md:flex">
             @auth
                 <a href="{{ route('dashboard') }}" wire:navigate
                     class="rounded-xl bg-[#16a34a] px-4 py-2 text-sm font-semibold text-white shadow-[0_10px_30px_rgba(2,44,20,0.10)] hover:bg-[#15803d] focus:outline-none focus:ring-2 focus:ring-[#bbf7d0]">
                     Dashboard
                 </a>
             @else
         ...
         <!-- Mobile hamburger -->
         <button
             class="md:hidden rounded-xl p-2 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-[#bbf7d0]"
    ...
    <!-- backdrop (klik di sini juga bisa close) -->
        ...
        <div class="mx-auto max-w-7xl"
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-[0_10px_30px_rgba(2,44,20,0.10)]"
                ...
                <div class="flex items-center justify-between px-4 py-3 sm:px-5">
                    ...
                    <button
                        class="rounded-xl p-2 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-[#bbf7d0]"
                         ...
                         <div class="mt-4 grid grid-cols-2 gap-2">
                             @auth
                                 <a href="{{ route('dashboard') }}" wire:navigate @click="closeMenu()"
                                     class="col-span-2 rounded-xl bg-[#16a34a] px-4 py-2 text-center text-sm font-semibold text-white shadow-[0_10px_30px_rgba(2,44,20,0.10)] hover:bg-[#15803d]">
                                     Dashboard
                                 </a>
                             @else
                                 ...
                                 <a href="{{ route('register') }}" wire:navigate @click="closeMenu()"
                                     class="rounded-xl bg-[#16a34a] px-4 py-2 text-center text-sm font-semibold text-white shadow-[0_10px_30px_rgba(2,44,20,0.10)] hover:bg-[#15803d]">
                                     Daftar
                                 </a>
                             @endauth
                         </div>
